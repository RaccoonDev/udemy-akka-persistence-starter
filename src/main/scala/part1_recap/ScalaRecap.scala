...
object ScalaRecap extends App {
  ...
  // FP
  val anIncremented: Int => Int = (x: Int) => x + 1
  anIncremented(1)

  List(1,2,3).map(anIncremented)
  // HOF: flatMap, filter
  // for-comprehensions
  ...
  future.onComplete {
    ...
    case Failure(exception) => println(s"Unable to get meaning of life because of ${exception.getMessage}")
  } // on SOME thread
  ...
  def receive: AkkaReceive = {
    case "something" => println("ha ha ha ha")
  ...
  "Lassie".bark // Using constructor as implicit function applied to the value

  // implicit organizations
  // local scope
  implicit val numberOrdering: Ordering[Int] = Ordering.fromLessThan(_ > _)
  List(1,2,3).sorted // implicit ordering will be taken from local scope, above => List(3,2,1)
